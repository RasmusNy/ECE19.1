...
signal = allData.data(1:1:400000,2); %start index =1; stop index=10000
time = allData.data(1:1:400000,1);   %start index =1; stop index=10000
%signal = 30 + 0.5*sin(2*pi*5000*time) + sin(2*pi*10000*time);% testing
...
%--------------LOG SCALE-----------------
figure(2)
loglog(f,4*result)                            % plot result in log scale
% axis([0,Fs/2,0,0.2]);   % Zoom in 
title('Amplitude Spectrum of the Signal');grid;
xlabel('Frequency (Hertz)')
ylabel('Magnitude')
